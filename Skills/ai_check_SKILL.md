...
|---------|------|---------|
| 1.4 | 2026-03-09 | Added mandatory em dash zero-tolerance enforcement: grep/search verification step before Step 7, source-file fix requirement for code-generated PDFs, explicit distinction between prose em dashes (remove) and table placeholder dashes (keep). |
...
5. **Kill em dashes — zero tolerance, no exceptions.** Every prose em dash (—) must be replaced before the file is produced. Replace with a period, comma, or colon depending on the sentence. There are exactly two cases where a dash-like character is acceptable: (a) a hyphen inside a compound word (e.g. "self-paced"), and (b) a standalone "—" in a table cell used as a placeholder meaning N/A or blank. Everything else gets replaced. This applies to content in text files, Word docs, PDFs, and — critically — Python/code source files that generate PDFs. If the content is generated from code, the em dashes must be removed from the source strings, not just noted.
...
### Step 6b — Em Dash Zero Check (MANDATORY — runs before Step 7, no exceptions)

Before producing any output file, run a literal search for the em dash character (—) in all prose content. For text/docx/pptx files, search the extracted text. For code-generated PDFs, search the source Python/script files.

**For text-based files:**
```python
em_dash_hits = [line for line in content.splitlines() if '—' in line]
# Filter out table placeholder rows (lines where — is the only non-whitespace content in a cell)
prose_hits = [line for line in em_dash_hits if line.strip() not in ['—', '| — |', '—']]
assert len(prose_hits) == 0, f"Em dashes still present: {prose_hits}"
```

**For code-generated PDFs (Python source files):**
```bash
grep -n " — " source_file.py | grep -v "^#"
# Must return zero results before proceeding
```

**If any prose em dashes are found:** Go back to Step 6 and fix them. Do not proceed to Step 7 until this check passes with zero hits. This is a hard gate — the file does not get produced until em dashes are gone.
